 ...
 * 
 * 웹어플리케이션 시작시 이벤트를 감지하여 
 * contextInitalized()를 실행 시킨다 -> 웹 어플리케이션의 초기화 작업을 수행할 수 있다 -> 대표적인 예 : Spring Framework의 Security 작업임
 * 
 * 웹 어플리케이션 종료 직전에 이벤트를 감지하여 
 * contextDestroyed() 를 실행 시킨다 -> 웹 어플리케이션 종료 직전에 수행해야 할 백업 작업 및 로깅 작업 등을 수행한다.
 *
 */
@WebListener
public class SecurityInitializer implements ServletContextListener {
    ...
    // 개별 웹 프로그램들은 DB 연동 없이 즉시 사용하게 하는 목적
    context.setAttribute("securityRoleInfo", "보안 ROLE 공통 기준 정보");
    System.out.println("contextInitialized 에서 보안 ROLE 공통 기준 정보 할당");
    }
