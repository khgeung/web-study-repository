...
public class FrontControllerServlet extends HttpServlet {
	...
	/**
	  ...
	  요청 처리 담당 객체(controller)를 받아온다. HandlerMapping Factory를 통해서
	  요청을 처리할 담당 Controller를 실행한다
	  실행 후 반환된 정보를 이용해 forward 또는 redirect 방식으로 응답하게 View로 제어를 이동한다.
	  예외 공통정책 : 예외 발생 시 사용자에게는 점검중 페이지를 제공하고 시스템 콘솔에서는 예외 발생 경로와 
	  				메시지를 출력한다.
	 */
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response)
		...
		//각 서블릿 또는 jsp에서 반복적으로 작업하던 것을 중앙 Front에서 한번에
		//효율적으로 제어, 유지보수성 향상
		}catch (Exception e) { 
			e.printStackTrace();
